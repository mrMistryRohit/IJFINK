...
import { Bell, BookOpenCheck, CheckCircle2, ChevronLeft, Clock3, Download, Eye, FileCheck2, FileText, History, Mail, RefreshCw, Search, ShieldCheck, Upload, UserCog, XCircle } from "lucide-react";
...
const EditorIntegrated = () => {
  ...
  ] as Array<[string, string | number, LucideIcon, string]>).map(([label, value, Icon, tone]) => <div key={label} className="rounded-2xl border bg-white p-5 shadow-sm"><div className="flex justify-between"><div><p className="text-sm font-bold text-slate-500">{label}</p><p className="mt-2 text-3xl font-extrabold">{value}</p></div><span className={`flex h-11 w-11 items-center justify-center rounded-xl ${tone}`}><Icon size={20} /></span></div></div>)}</div></section>;
  ...
  else content = <section><Heading title="Editor Profile" text="Your authenticated editorial workspace identity." /><div className="rounded-2xl border bg-white p-6 shadow-sm"><div className="rounded-2xl bg-gradient-to-r from-slate-950 to-emerald-950 p-8 text-center text-white"><ShieldCheck size={36} className="mx-auto" /><h2 className="mt-4 text-2xl font-extrabold">{user?.display_name || "Editor"}</h2><p className="text-emerald-300">Editor</p></div><div className="mt-5 grid gap-4 md:grid-cols-3">{([[Mail, "Email", user?.email || "Not available"], [ShieldCheck, "Status", user?.status || "Not available"], [UserCog, "Profile ID", String(user?.profile_id || "Not available")]] as Array<[LucideIcon, string, string]>).map(([Icon, label, value]) => <div key={label} className="rounded-xl bg-slate-50 p-4"><Icon size={18} className="text-primary" /><p className="mt-3 text-xs font-bold uppercase text-slate-400">{label}</p><p className="mt-1 break-words font-bold">{value}</p></div>)}</div></div></section>;
