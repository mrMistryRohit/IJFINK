...
import { Activity, BarChart3, Bell, BookOpenCheck, CheckCircle2, ChevronLeft, Clock3, Download, FileText, History, Mail, RefreshCw, Search, ShieldCheck, Upload, UserCog, Users, XCircle } from "lucide-react";
...
import { submitEditorReview, uploadEditorEditableManuscript } from "@/lib/editorApi";
...
const ChiefEditorIntegrated = () => {
  ...
  const download = async (fileId: number, fileName: string) => {
    if (!detail) return;
    setActing(true);
    try { const blob = await downloadChiefArticleFile(detail.article.article_id, fileId); const url = URL.createObjectURL(blob); const link = document.createElement("a"); link.href = url; link.download = fileName; link.click(); window.setTimeout(() => URL.revokeObjectURL(url), 60000); }
  ...
  if (loading) content = <WorkspaceLoading />;
  ...
  ].map(([label, value, Icon]) => <div key={String(label)} className="rounded-2xl border bg-white p-5 shadow-sm"><div className="flex justify-between"><div><p className="text-sm font-bold text-slate-500">{String(label)}</p><p className="mt-2 text-3xl font-extrabold">{String(value)}</p></div><span className="flex h-11 w-11 items-center justify-center rounded-xl bg-primary/10 text-primary"><Icon size={20} /></span></div></div>)}</div></section>;
    ...
    <h2 className="mt-7 font-extrabold">Files</h2><div className="mt-3 grid gap-3 md:grid-cols-2">{detail.files.map((file) => <div key={file.file_id} className="flex items-center gap-3 rounded-xl border p-4"><FileText size={18} className="text-primary" /><div className="min-w-0 flex-1"><p className="truncate text-sm font-bold">{file.file_name}</p><p className="text-xs text-slate-500">{file.file_type} / Version {file.version}</p></div><button disabled={acting} onClick={() => void download(file.file_id, file.file_name)} className="p-2 text-primary"><Download size={17} /></button></div>)}</div>
    <div className="mt-7 grid gap-5 lg:grid-cols-2"><div className="rounded-2xl border p-5"><h2 className="font-extrabold">Upload edited manuscript</h2><input type="file" accept=".doc,.docx,.odt,.rtf" onChange={(event) => setEditableFile(event.target.files?.[0] || null)} className="mt-4 block w-full text-sm" /><button disabled={!editableFile || acting} onClick={() => void upload()} className="mt-4 inline-flex items-center gap-2 rounded-xl bg-slate-900 px-4 py-2.5 text-sm font-bold text-white disabled:opacity-50"><Upload size={16} /> Upload version</button></div><div className="rounded-2xl border p-5"><h2 className="font-extrabold">Editorial intervention</h2><select value={decision} onChange={(event) => setDecision(event.target.value)} className="mt-4 h-11 w-full rounded-xl border px-3 text-sm"><option>Accepted</option><option>Minor Revision</option><option>Major Revision</option><option>Rejected</option></select><textarea value={comments} onChange={(event) => setComments(event.target.value)} placeholder="Comments..." className="mt-3 min-h-24 w-full rounded-xl border p-3 text-sm" /><button disabled={acting} onClick={() => void submitDecision()} className="mt-3 rounded-xl bg-primary px-4 py-2.5 text-sm font-bold text-white">Submit decision</button></div></div>
