...
const PublicationOverview = ({ accepted, published, isLoading, onOpenQueue, onOpenPublished }: Props) => {
  ...
  return <section>
    ...
    <div className="grid gap-4 md:grid-cols-3">{cards.map((card) => <article key={card.label} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><div className={`flex h-11 w-11 items-center justify-center rounded-xl ${card.tone}`}><card.icon size={21} /></div><p className="mt-5 text-3xl font-extrabold text-slate-950">{isLoading ? "0" : card.value}</p><p className="mt-1 text-sm font-semibold text-slate-500">{card.label}</p></article>)}</div>
    <div className="mt-5 grid gap-5 lg:grid-cols-2">
      ...
      <article className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm"><h2 className="font-extrabold text-slate-950">Recent publications</h2><p className="mt-1 text-sm text-slate-500">Latest completed publication records.</p><div className="mt-5 space-y-3">{published.slice(0, 3).map((item) => <div key={item.publication_id} className="rounded-xl border border-slate-100 bg-slate-50 p-3"><p className="line-clamp-1 text-sm font-bold text-slate-900">{item.title || `Article #${item.article_id}`}</p><p className="mt-1 text-xs text-slate-500">{item.doi} | {formatApiDateTimeIST(item.publication_date)}</p></div>)}{!isLoading && published.length === 0 && <p className="py-5 text-center text-sm text-slate-500">No publications are recorded yet.</p>}</div><button onClick={onOpenPublished} className="mt-5 inline-flex items-center gap-2 text-sm font-bold text-primary">View archive <ArrowRight size={16} /></button></article>
    </div>
  </section>;
};
