...
const fileLabels: Array<{
  ...
  {
    key: "editableManuscript",
    label: "Editable Manuscript",
    helper: "DOCX preferred",
  ...
  },
  {
    ...
    helper: "Any supporting material",
    accept: ".pdf,.doc,.docx,.zip,.csv,.xlsx,.xls,.png,.jpg,.jpeg,application/pdf,application/zip",
    multiple: true,
  },
  {
    key: "videoAbstract",
    label: "Video Abstract",
    helper: "MP4, MOV, or AVI",
...
const emptyFiles = (): SubmissionFiles => ({
  mainManuscript: null,
  editableManuscript: null,
  coverLetter: null,
  figures: [],
  supplementaryFiles: [],
  videoAbstract: null,
...
const buildSubmissionPayload = (
  ...
  coAuthors: CoAuthorForm[],
  selectedFiles: SubmissionFiles,
) => {
  ...
  if (selectedFiles.mainManuscript) {
    formData.append("main_manuscript", selectedFiles.mainManuscript);
  }
  if (selectedFiles.editableManuscript) {
    formData.append("editable_manuscript", selectedFiles.editableManuscript);
  }
  if (selectedFiles.coverLetter) {
    formData.append("cover_letter", selectedFiles.coverLetter);
  }
  ...
  if (selectedFiles.videoAbstract) {
    formData.append("video_abstract", selectedFiles.videoAbstract);
  }
  if (selectedFiles.copyrightForm) {
    formData.append("copyright_form", selectedFiles.copyrightForm);
  }
...
const SubmitPublicationForm = () => {
  ...
  if (activeStepIndex < 0) {
    return <Navigate to={`${SUBMIT_ROOT}/${DEFAULT_STEP}`} replace />;
  }
  ...
  const account = {
    ...
    researchArea: userProfile.researchArea,
  };

  const updateSubmissionDetails = (field: keyof SubmissionDetails, value: string) => {
    setSubmissionDetails((currentDetails) => ({
      ...currentDetails,
      [field]: value,
    }));
  };

  const updateCoAuthor = (index: number, field: keyof CoAuthorForm, value: string) => {
    setCoAuthors((currentAuthors) =>
  ...
  const addCoAuthor = () => {
    setCoAuthors((currentAuthors) => [...currentAuthors, emptyCoAuthor()]);
  };

  const removeCoAuthor = (index: number) => {
    setCoAuthors((currentAuthors) => currentAuthors.filter((_, authorIndex) => authorIndex !== index));
  };
  ...
  const validateAuthors = () => {
    ...
    for (const author of activeCoAuthors) {
      ...
      if (author.orcid.trim() && !/^\d{4}-\d{4}-\d{4}-\d{3}[\dX]$/.test(author.orcid.trim())) {
        return "ORCID must follow the 0000-0000-0000-0000 format.";
      }
    }
    ...
    for (const author of activeCoAuthors) {
      ...
      if (uniqueEmails.has(normalizedEmail)) {
        return "Co-author emails must be unique.";
      }
  ...
  const validateFiles = () => {
    if (!selectedFiles.mainManuscript) return "Main Manuscript is required.";
    return "";
  };

  const validateDeclarations = () => {
    if (selectedDeclarations.length !== declarations.length) {
      return "Please confirm all declarations before submitting.";
    }
    return "";
  };
  ...
  const handleSubmit = async () => {
    ...
    if (!token) {
      toast({
        ...
        description: "Please sign in before submitting a paper.",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      const response = await fetch(`${normalizeApiBaseUrl(API_BASE_URL)}/api/user/articles`, {
        method: "POST",
        headers: {
      ...
      if (!response.ok || !responseData?.success) {
        throw new Error(responseData?.message ?? "The backend rejected the submission.");
      }

      toast({
        title: "Article submitted",
        description: "Your manuscript has been sent for editorial screening.",
      });
      ...
      toast({
        ...
        description: error instanceof Error ? error.message : "Something went wrong while submitting.",
        variant: "destructive",
      });
    } finally {
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <div className="md:col-span-2">
          ...
          <input
            ...
            onChange={(event) => updateSubmissionDetails("title", event.target.value)}
        ...
        <div>
          ...
          <select
            className={fieldClass}
            value={submissionDetails.articleType}
            onChange={(event) => updateSubmissionDetails("articleType", event.target.value)}
          >
        ...
        <div>
          ...
          <input
            className={fieldClass}
            value={submissionDetails.subjectArea}
            onChange={(event) => updateSubmissionDetails("subjectArea", event.target.value)}
        ...
        <div className="md:col-span-2">
          ...
          <textarea
            ...
            value={submissionDetails.abstract}
            onChange={(event) => updateSubmissionDetails("abstract", event.target.value)}
        ...
        <div className="md:col-span-2">
          ...
          <input
            className={fieldClass}
            value={submissionDetails.keywords}
            onChange={(event) => updateSubmissionDetails("keywords", event.target.value)}
    ...
    <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
      ...
      <div className="mt-4 space-y-4">
        ...
        {coAuthors.map((author, index) => (
          <div key={`${index}-${author.email}`} className="rounded-2xl border border-slate-100 bg-slate-50 p-4">
            <div className="flex items-center justify-between gap-3">
              <h3 className="text-sm font-extrabold uppercase tracking-wide text-slate-700">Co-author {index + 1}</h3>
              <button
                type="button"
                onClick={() => removeCoAuthor(index)}
                className="inline-flex items-center gap-1 rounded-full px-2 py-1 text-xs font-bold text-rose-600 hover:bg-rose-50"
              >
            ...
            <div className="mt-4 grid gap-4 md:grid-cols-2">
              <div className="md:col-span-2">
                ...
                <input
                  ...
                  onChange={(event) => updateCoAuthor(index, "fullName", event.target.value)}
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(event) => updateCoAuthor(index, "institution", event.target.value)}
                  placeholder="Institution / affiliation"
                />
              </div>
              <div className="md:col-span-2">
                <label className="mb-2 block text-sm font-bold text-slate-700">ORCID</label>
                <input
                  className={fieldClass}
                  value={author.orcid}
                  onChange={(event) => updateCoAuthor(index, "orcid", event.target.value)}
      ...
      <div className="grid gap-4 md:grid-cols-2">
        {fileLabels.map((fileField) => {
          ...
          return (
            <div
              key={fileField.key}
              className={fileField.multiple || fileField.key === "coverLetter" ? "md:col-span-2" : ""}
            >
              <div className="mb-2 flex items-center justify-between gap-3">
                <label className="text-sm font-extrabold text-slate-800">{fileField.label}</label>
                <span className={`text-xs font-semibold ${fileField.required ? "text-rose-600" : "text-slate-400"}`}>
                  {fileField.helper}
                </span>
              </div>
              <label className="flex min-h-28 cursor-pointer flex-col items-center justify-center rounded-2xl border border-dashed border-slate-300 bg-slate-50 p-4 text-center transition-colors hover:border-primary hover:bg-primary/5">
                ...
                <input
                  ...
                  className="hidden"
                  onChange={(event) => updateFile(fileField.key, event.target.files)}
                />
              </label>
            </div>
          );
  ...
  const renderReviewStep = () => {
    const summaryKeywords = submissionDetails.keywords
      .split(",")
      .map((keyword) => keyword.trim())
      .filter(Boolean);
    const activeCoAuthors = coAuthors.filter(isFilledCoAuthor);
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        ...
        <div className="mt-6 space-y-5">
          <div className="rounded-xl border border-slate-100 bg-slate-50 p-4">
            <h3 className="font-extrabold text-slate-950">Article Details</h3>
            <div className="mt-4 grid gap-3 md:grid-cols-2">
              <div>
                <p className="text-xs font-bold uppercase text-slate-400">Article title</p>
                <p className="mt-1 text-sm font-semibold text-slate-800">
                  {submissionDetails.title || "Not provided"}
                </p>
              </div>
              <div>
                <p className="text-xs font-bold uppercase text-slate-400">Article type</p>
                <p className="mt-1 text-sm font-semibold text-slate-800">
                  {submissionDetails.articleType || "Not provided"}
                </p>
              </div>
              <div>
                <p className="text-xs font-bold uppercase text-slate-400">Subject area</p>
                <p className="mt-1 text-sm font-semibold text-slate-800">
                  {submissionDetails.subjectArea || "Not provided"}
                </p>
              </div>
              <div>
                <p className="text-xs font-bold uppercase text-slate-400">Keywords</p>
                <p className="mt-1 text-sm font-semibold text-slate-800">
                  {summaryKeywords.length ? summaryKeywords.join(", ") : "Not provided"}
                </p>
              </div>
              <div className="md:col-span-2">
                <p className="text-xs font-bold uppercase text-slate-400">Abstract</p>
                <p className="mt-1 max-h-24 overflow-hidden text-sm leading-relaxed text-slate-700">
                  {submissionDetails.abstract || "Not provided"}
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="rounded-xl border border-slate-100 bg-slate-50 p-4">
            <h3 className="font-extrabold text-slate-950">Co-Authors</h3>
            <div className="mt-4 space-y-3">
              {activeCoAuthors.length === 0 ? (
                <p className="text-sm text-slate-500">No co-authors added.</p>
              ) : (
                activeCoAuthors.map((author, index) => (
                  <div key={`${author.email}-${index}`} className="rounded-xl border border-slate-100 bg-white p-4">
                    <p className="font-extrabold text-slate-900">{author.fullName}</p>
                    <div className="mt-2 grid gap-2 md:grid-cols-2">
                      <p className="text-sm text-slate-600">Email: {author.email}</p>
                      <p className="text-sm text-slate-600">Institution: {author.institution}</p>
                      <p className="text-sm text-slate-600 md:col-span-2">
                        ORCID: {author.orcid || "Not provided"}
                      </p>
                    </div>
                  </div>
                ))
          ...
          <div className="rounded-xl border border-slate-100 bg-slate-50 p-4">
            ...
            <div className="mt-4 grid gap-3 md:grid-cols-2">
              {fileLabels.map((file) => (
                <div key={file.key} className="rounded-xl border border-slate-100 bg-white p-4">
                  <p className="text-xs font-bold uppercase text-slate-400">{file.label}</p>
                  <p className="mt-1 text-sm font-bold text-slate-800">
                    {getFileNames(selectedFiles[file.key] as File[] | File | null) || "No file selected"}
                  </p>
                </div>
              ))}
            </div>
  ...
  const renderConfirmStep = () => (
    <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
      <div className="flex items-center gap-2">
        <CheckCircle2 size={18} className="text-primary" />
        <h2 className="text-lg font-extrabold text-slate-950">Final Confirmation</h2>
      </div>
    ...
    <section className="space-y-6">
      <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        ...
        <p className="mt-2 max-w-3xl text-sm leading-relaxed text-slate-500">
          This is now a step-by-step submission flow. Each page is a separate route so authors can move through the
          backend-aligned process one screen at a time.
        </p>
      </div>
      ...
      <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_320px]">
        <div className="space-y-6">
          ...
          {activeStepKey === "authors" && renderAuthorsStep()}
          {activeStepKey === "files" && renderFilesStep()}
          {activeStepKey === "review" && renderReviewStep()}
          {activeStepKey === "confirm" && renderConfirmStep()}
          ...
          <div className="flex flex-wrap items-center justify-between gap-3">
            ...
            {activeStepKey !== "confirm" ? (
              ...
              <button
                ...
                className="inline-flex items-center justify-center gap-2 rounded-xl bg-primary px-6 py-3 text-sm font-bold text-white transition-colors hover:bg-primary/90 disabled:cursor-not-allowed disabled:opacity-60"
              >
                <Send size={17} />
        ...
        <aside className="space-y-5">
          <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
            <h2 className="font-extrabold text-slate-950">Submission Pages</h2>
            <div className="mt-4 space-y-3">
                ...
                return (
                  <button
                    ...
                    className={`flex w-full items-center gap-3 rounded-xl p-3 text-left transition-colors ${
                      isActive ? "bg-primary/10" : "bg-slate-50 hover:bg-slate-100"
                    }`}
                  >
                    <div
                    ...
                    <div>
                      ...
                      <p className="text-xs text-slate-500">{step.help}</p>
                    </div>
                  </button>
                );
              })}
